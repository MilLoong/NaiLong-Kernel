...
/**
 * GCC 15+ 在启用 _GLIBCXX_ASSERTIONS 时，<atomic> 会引用该符号。
 * freestanding / -nostdlib 内核不链接 libstdc++，需自行提供。
 */
namespace std {
[[noreturn]] auto __glibcxx_assert_fail(const char* file, int line,
                                        const char* function,
                                        const char* condition) noexcept -> void {
  (void)file;
  (void)line;
  (void)function;
  (void)condition;

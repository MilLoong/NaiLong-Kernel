...
在内核启动的最早期（在此阶段，内存管理、中断甚至设备树解析都尚未完成），为了能够打印调试信息，NaiLong-Kernel 提供了 Early Console 机制。

### 开启方式

通过 CMake 预设变量 `NAILONG_KERNEL_EARLY_CONSOLE` 启用。该变量定义了串口控制器的物理基地址。
...
利用 C++ 静态全局对象的构造函数在 `main` 函数之前执行的特性，`EarlyConsole` 类的构造函数会初始化一个最小可用的串口驱动，并设置 `nk_putchar` 函数指针，从而使得 `klog` 等日志函数能够立即工作。
...
extern "C" void nk_putchar(int c, [[maybe_unused]] void *ctx) {
  if (serial) {
    serial->Write(c);
  }
}

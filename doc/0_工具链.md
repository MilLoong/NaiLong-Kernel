...
NaiLong-Kernel 为了保证在各个平台上的可用性，选择了较为通用的工具链，主要分为四个部分：
...
NaiLong-Kernel 采用现代化的 CMake 构建系统，具有以下特点：
...
NaiLong-Kernel 支持多架构交叉编译，需要根据目标平台配置相应的 GCC 工具链。
...
QEMU 是 NaiLong-Kernel 的主要运行环境，支持所有目标架构的模拟。
...
```bash
# 从仓库内 Dockerfile 构建开发镜像
docker build -t nailong-kernel-dev:latest --target nailong_kernel ./tools
docker run -it --rm -v "$PWD":/root/NaiLong-Kernel nailong-kernel-dev:latest
```
